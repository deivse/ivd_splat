...
for strategy in $ALL_STRATEGIES; do
    ...
    ## Additional experiment where we also include the SfM points
    train_strat_with_laser_scan $FINAL_NUM_POINTS_PER_SCENE_FILE "dense_init.include_sparse={True}"
    train_strat_with_laser_scan $FINAL_NUM_POINTS_PER_SCENE_FILE "dense_init.include_sparse={True} dense_init.target_points_fraction={$INIT_FRACTIONS}"
