	...
	bool prefiltered)
{
	...
	uint32_t tiles_count = duplicateToTilesTouched(point_image, con_o, grid, 0, 0, 0, nullptr, nullptr);
    if (tiles_count == 0)
        return;
	...
	float* __restrict__ dist_accum)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			{
				atomicAdd(&accum_weights[collected_id[j]], pixel_weights[pix_id] * T * alpha);
