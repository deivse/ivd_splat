# License Notice: Based on gsplat code, which is licensed under Apache 2.0.
...
@dataclass
class DefaultWithGaussianCapStrategy(IVDSplatBaseStrategy):
    ...
    cap_max: int = -1  # Maximum number of GSs allowed. -1 means no cap.
    ...
    def _grow_gs(
        ...
        num_splats_to_add = min(
            max(self.cap_max - curr_num_splats, 0), is_grad_high.sum().item()
        )
        if self.cap_max != -1 and num_splats_to_add < is_grad_high.sum().item():
            # keep only the top-k splats based on gradient magnitude
            _, topk_indices = torch.topk(grads[is_grad_high], num_splats_to_add)
            new_is_grad_high = torch.zeros_like(is_grad_high, dtype=torch.bool)
            new_is_grad_high[torch.where(is_grad_high)[0][topk_indices]] = True
            is_grad_high = new_is_grad_high
